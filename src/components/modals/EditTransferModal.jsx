...
import { updateTransferRequest } from '../../api/transfers';

const EditTransferModal = (props) => {
    ...
    const onSubmit = handleSubmit(async (data) => {
        console.log(data);
        try {
            const res = await updateTransferRequest(props.dataTransfer._id ,data);
            console.log(res);
            if (res.data) {
                props.setShowModalEditTransfer(false);
                props.getTransfer();
            }
        } catch (error) {
            console.log(error);
    ...
    useEffect(() => {
        getFlows();
        getCards();
        console.log(props.dataTransfer)
        if(props.dataTransfer){
            setValue('concept', props.dataTransfer.concept);
            setValue('amount', props.dataTransfer.amount);
            setValue('flow', props.dataTransfer.flow);
            setValue('card', props.dataTransfer.card);
        }
    }, []);
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                            ...
                            <button
                                ...
                                onClick={() => props.setShowModalEditTransfer(false)}
                            >
