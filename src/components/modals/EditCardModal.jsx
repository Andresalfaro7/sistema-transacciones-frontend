...
const EditCardModal = (props) => {
    ...
    useEffect(() => {
        if(props.dataCard){
            setValue('number', dataCard.number);
            setValue('cardholder', dataCard.cardholder);
            setValue('expirationDate', dataCard.expirationDate);
            setValue('cvv', dataCard.cvv);
            setValue('totalAmount', dataCard.totalAmount);
        }
    }, []);

    const onSubmit = handleSubmit(async (data) => {
        const res = await updateCard(dataCard._id, data);
        console.log(res);
        if (res.data) {
            props.setShowModalEditCard(false);
            props.getCards();
        }
    });
                    ...
                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                        ...
                        <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                            ...
                            <button
                                ...
                                onClick={() => props.setShowModalEditCard(false)}
                            >
